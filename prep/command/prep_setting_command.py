...
from prep.prep.prep import create_writer_by_args
...
def _get_prep_info_from_cli():
    ...
    while True:
        try:
            ...
            check_name_format(prep_info['name'])

            prep_info['country'] = input('> country : ')
            check_country_format(prep_info['country'])

            prep_info['city'] = input('> city : ')
            check_city_format(prep_info['city'])

            prep_info['email'] = input('> email : ')
            check_email_format(prep_info['email'])

            prep_info['website'] = input('> website : ')
            check_website_format(prep_info['website'])

            prep_info['details'] = input('> details : ')
            check_details_format(prep_info['details'])

            prep_info['p2pEndpoint'] = input('> p2pEndpoint : ')
            check_p2p_endpoint_format(prep_info['p2pEndpoint'])
