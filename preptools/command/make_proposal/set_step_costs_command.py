...
class SetStepCostsCommand(Command):
    _options = (
        "default",
        "contract-call",
        "contract-create",
    ...
    def _run(self, args: Namespace) -> str:
        ...
        for option in self._options:
            ...
            value: Optional[int] = getattr(args, option)
            if value is not None:
                params = [{"type": "int", "value": value}]
                proposal: str = self._make_proposal(ZERO_ADDRESS, "setStepCost", params, raw_json=True)
                proposals.append(proposal)

        proposals_str = json.dumps(proposals)
        self._write_proposal(args.output, proposals_str)
